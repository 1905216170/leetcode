...
public class BM37_二叉搜索树的最近公共祖先 {
    ...
    public int findLowestCommonAncestor(TreeNode node, int p, int q){
        ...
        // 递归到最深处，再从叶子节点往上回退查找
        // node.val < p && node.val <q ? 0是二叉查找树的优化，如果p和q都大于当前节点，那么就没有必要遍历左子树
        int type1 = node.val < p && node.val < q ? 0 : findLowestCommonAncestor(node.left, p, q);
        int type2 = node.val > p && node.val > q ? 0 : findLowestCommonAncestor(node.right, p, q);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new BM37_二叉搜索树的最近公共祖先().lowestCommonAncestor(node1, 11, 12));
    }
